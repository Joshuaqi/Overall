...
public final class LocalBroadcastManager {
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                AppLogger.v("LocalBroadcastManager"+"Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }

            ArrayList entries = (ArrayList) this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    AppLogger.v("LocalBroadcastManager"+"Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        AppLogger.v("LocalBroadcastManager"+ "Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            AppLogger.v("LocalBroadcastManager"+ "  Filter\'s target already added");
                        }
                    } else {
                        int match = receiver.filter.match(action, type, scheme, data, categories, "LocalBroadcastManager");
                        if (match >= 0) {
                            if (debug) {
                                AppLogger.v("LocalBroadcastManager"+ "  Filter matched!  match=0x" + Integer.toHexString(match));
                            }
                            ...
                            AppLogger.v("LocalBroadcastManager"+"  Filter did not match: " + reason);
                        }
                    }
                }
